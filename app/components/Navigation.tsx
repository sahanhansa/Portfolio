  ...
  return (
    <nav className="fixed top-0 w-full bg-dark bg-opacity-95 backdrop-blur-sm border-b border-gray-800 z-50" style={{paddingTop: 'env(safe-area-inset-top)'}}>
      <div className="container max-w-full px-2 sm:px-4">
        <div className="flex flex-col lg:flex-row items-center justify-between h-auto lg:h-16 py-2 lg:py-0">
          <a 
            href="#home" 
            className="text-lg sm:text-xl font-bold text-primary hover:text-opacity-80 transition-colors duration-300 whitespace-nowrap flex-shrink-0 mb-2 lg:mb-0" 
            onClick={(e) => { e.preventDefault(); scrollToSection('home') }}
          >
            Sahan Siriwardhana
          </a>
          <div className="flex flex-col lg:flex-row items-center w-full lg:w-auto">
            <ul className={`flex flex-col lg:flex-row items-center w-full lg:w-auto space-y-2 lg:space-y-0 lg:space-x-4 xl:space-x-6 ${isMenuOpen ? 'flex' : 'hidden lg:flex'}`}>
              <li>
                <a 
                  href="#home" 
                  className={`text-xs sm:text-sm xl:text-base font-medium transition-colors duration-300 hover:text-primary relative whitespace-nowrap ${activeSection === 'home' ? 'text-primary' : 'text-text-secondary'}`}
                  onClick={(e) => { e.preventDefault(); scrollToSection('home') }}
                >
                  Introduction
                  {activeSection === 'home' && (
            ...
            <a 
              href="#contact" 
              className="px-3 py-2 bg-blue-600 text-white text-xs sm:text-sm rounded font-medium hover:bg-blue-700 transition-colors duration-300 mt-2 lg:mt-0 hidden lg:inline-block whitespace-nowrap flex-shrink-0"
            ...
            <button 
              className="lg:hidden w-8 h-8 flex flex-col justify-center items-center space-y-1 flex-shrink-0 ml-auto"
